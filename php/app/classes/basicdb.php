...
class basicdb extends \PDO
{
    ...
    public function orHaving($column, $value, $mark = '=')
    {
    ...
    private function getWhere($conditionType = 'where')
    {
        if (
        ...
        if ($this->totalRecord > $this->paginationLimit) {
            for ($i = $this->page - 2; $i < $this->page + 2 + 1; $i++) {
                if ($i > 0 && $i <= $this->pageCount) {
                    if ($bootstrap) {
                        $this->html .= '<li class="page-item ' . ($i == $this->page ? $class : null) . '">' .
                            '<a class="page-link" href="' . str_replace('[page]', $i, $url) . '">' . $i . '</a>' .
                            '</li>';
                    } else {
