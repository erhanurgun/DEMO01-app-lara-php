$(function () {
    ...
    tinymce.init({
        ...
        external_filemanager_path: appUrl + "/3rd-party-apps/filemanager/",
        filemanager_title: "Dosya Yöneticisi",
        external_plugins: {"filemanager": appUrl + "/3rd-party-apps/filemanager/plugin.min.js"},
        filemanager_access_key: "erhanrgn_04",
    ...
    // tablola kolonlarını kendi aralarında sıralama
    $('.table-sortable').sortable({
        update: function (event, ui) {
            ...
            $.post(apiUrl + '/table-sort', postData, function (response) {
                if (response.success) {
                    $('.sortable-mssg.success').show().find('>div').html(response.success);

                } else if (response.error) {
                    $('.sortable-mssg.error').show().find('>div').html(response.error);
                }
                setTimeout(function () {
                    $('.sortable-mssg.success').hide();
                    $('.sortable-mssg.error').hide();
                }, 3500);
            }, 'json');
        }
    });
    ...
    // tags etiketerini görselleştirme
    $('.tagsinput').tagsInput({
